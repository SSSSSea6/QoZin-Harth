...
- [x] 工具市场：AI 审核上架、管理员复核与试运行、按圈安装、权限清单、独立源沙箱
- [x] 工具的后端代码与定时任务：沙箱运行、圈主确认时间表、运行记录
- [x] 开发者资格与额度：发布需要邀请码或申请；按资源用量计的每月免费燃料，用完暂停到下月
- [x] 实名与治理：发言前绑定手机号；举报、屏蔽、隐藏、禁言、封禁、申诉，每步留记录；工具要拿成员数据先经本人同意
- [x] 通知收件箱：私聊、工具发帖、圈子快熄、申请与举报结果；App 推送的服务端（APNs / EMAS）已就位
- [ ] 工具访问站外服务
- [ ] Expo App
...
- 想给火塘做一个工具？看站内的开发者页（`/developers`）：在自己的圈里 `harth dev` 试跑不需要资格，发布需要邀请码或申请。
